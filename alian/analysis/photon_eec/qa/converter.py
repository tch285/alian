...
class TrackQA:
    ...
    def create_histos(self):
        ...
        self.histos['track_pt'] = self.df_track.Histo1D(
            ("track_pt", 'Track #it{p}_{T};track #it{p}_{T} (GeV);Counts', 200, linbins(0, 200, 200)),
            "track_pt_selected"
        )
        self.histos['track_eta'] = self.df_track.Histo1D(
            ("track_eta", 'Track #eta;track #eta;Counts', 100, -1, 1),
            "track_eta_selected"
        )
        self.histos['track_phi'] = self.df_track.Histo1D(
            ("track_phi", 'Track #phi;track #phi;Counts', 80, -1, 7),
            "track_phi_selected"
        )
        self.histos['cluster_eta'] = self.df.Histo1D(
            ("cluster_eta", 'Cluster #eta;cluster #eta;Counts', 100, -1, 1),
            "cluster_eta"
        )
        self.histos['cluster_phi'] = self.df.Histo1D(
            ("cluster_phi", 'Cluster #phi;cluster #phi;Counts', 80, -1, 7),
            "cluster_phi"
        )
        ...
        # self.histos['track_eta_phi_pt'] = self.df_track.Histo3D(
...
if __name__ == '__main__':
    ...
    analysis = TrackQA(args.input, args.output, args.config)
    analysis.analyze()