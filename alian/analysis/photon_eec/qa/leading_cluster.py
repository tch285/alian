...
class LeadingClusterAnalysis(AnalysisBase):
    ...
    def analyze_event(self):
        ...
        pts = [c.pt() for c in self.clusters]
        if not pts:
            pt_max = -1
        else:
            pt_max = max(pts)
        self.hists['clus_lead_pt'].Fill(pt_max)

        # if there is at least one cluster above the minimum, fill histo with pT of all clusters
        if [pt for pt in pts if pt > self.clus_pT_min]:
            [self.hists['clus_pt'].Fill(pt) for pt in pts]
