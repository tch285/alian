...
def shouldSelectClusterAsPhotonCandidate(cluster: Cluster, tracks: List[Track]) -> bool:
    # Cluster shape
    if not (cluster.m02 > cuts.cluster.m02_min and cluster.m02 < cuts.cluster.m02_max):
        hists.fillQAStatistics("Number of clusters exluded due to shape", 1)
        return False
    hists.fillClusterQA(cluster, "After Cluster Shape Cut")
    # Cluster time
    if not (cluster.time > cuts.cluster.time_min and cluster.time < cuts.cluster.time_max):
    ...
    if cluster.ncells < cuts.cluster.ncells_min:
    ...
    # Cluster energy
    if cluster.energy < cuts.cluster.E_min:
    ...
    for track in tracks:
        if doTrackClusterMatch(cluster, track):
            if cluster.energy / track.p <= cuts.cluster.energy_pt_ratio_threshold:
...
def isIsolatedPhotonCandidate(candidate: Cluster, tracks: List[Track]) -> bool:
    ...
    for track in tracks:
        ...
        if np.sqrt((candidate.eta - track.eta) ** 2 + (delta_phi ** 2)) < cuts.cluster.iso_cone_radius:
            total_pt += track.pt
    # Fill corresponding histogram
    hists.h_total_track_pt_in_isolation_cone.Fill(total_pt)
    if total_pt >= cuts.cluster.iso_pt_threshold:
        return False
    return True
