// SimpleTimeShowerCustomized.cc is a part of the PYTHIA event generator.
// Copyright (C) 2023 Torbjorn Sjostrand.
// PYTHIA is licenced under the GNU GPL v2 or later, see COPYING for details.
// Please respect the MCnet Guidelines, see GUIDELINES for details.

// Function definitions (not found in the header) for the
// SimpleTimeShowerCustomized class.
...
// The SimpleTimeShowerCustomized class.
...
const double SimpleTimeShowerCustomized::MCMIN         = 1.2;
const double SimpleTimeShowerCustomized::MBMIN         = 4.0;

// For small x approximate 1 - sqrt(1 - x) by x/2.
const double SimpleTimeShowerCustomized::SIMPLIFYROOT  = 1e-8;
...
const double SimpleTimeShowerCustomized::XMARGIN       = 1e-12;
const double SimpleTimeShowerCustomized::XMARGINCOMB   = 1e-4;
...
const double SimpleTimeShowerCustomized::WTRATIOMAX    = 1.01;

// Lower limit on PDF value in order to avoid division by zero.
const double SimpleTimeShowerCustomized::TINYPDF       = 1e-10;

// Big starting value in search for smallest invariant-mass pair.
const double SimpleTimeShowerCustomized::LARGEM2       = 1e20;

// In g -> q qbar or gamma -> f fbar require m2_pair > this * m2_q/f.
const double SimpleTimeShowerCustomized::THRESHM2      = 4.004;

// Never pick pT so low that alphaS is evaluated too close to Lambda_3.
const double SimpleTimeShowerCustomized::LAMBDA3MARGIN1ORD = 1.1;
const double SimpleTimeShowerCustomized::LAMBDA3MARGIN2ORD = 1.6;

// Max loop counter
const int    SimpleTimeShowerCustomized::NLOOPMAX = 10000;
...
const bool   SimpleTimeShowerCustomized::FIXRESCATTER          = true;
// Veto negative energies when using FIXRESCATTER option.
const bool   SimpleTimeShowerCustomized::VETONEGENERGY         = false;
// Do not allow too large time- or spacelike virtualities in fixing-up.
const double SimpleTimeShowerCustomized::MAXVIRTUALITYFRACTION = 0.5;
// Do not allow too large negative spacelike energy in system rest frame.
const double SimpleTimeShowerCustomized::MAXNEGENERGYFRACTION  = 0.7;

// Fudge extra weight for overestimation of weak shower t-channel correction.
const double SimpleTimeShowerCustomized::WEAKPSWEIGHT = 5.;

// Extra overestimate of g -> q qbar branching rate for DGLAP comparison.
const double SimpleTimeShowerCustomized::WG2QEXTRA = 20.;

// Limit on size of number of rejections for uncertainty variations.
const double SimpleTimeShowerCustomized::REJECTFACTOR = 0.1;

// Limit on probability for uncertainty variations.
const double SimpleTimeShowerCustomized::PROBLIMIT = 0.99;

//--------------------------------------------------------------------------

// Initialize alphaStrong, alphaEM and related pTmin parameters.

void SimpleTimeShowerCustomized::addParameters(Settings &settings)
{
  settings.addParm("SimpleTimeShowerCustomized:zBias", 0.0, true, false, 0.0, 1.0);
  settings.addParm("SimpleTimeShowerCustomized:coherenceFactor", 0.0, true, false, 0.0, 1.0);
  settings.addParm("SimpleTimeShowerCustomized:azimuthalBias", 0.0, true, false, 0.0, 1.0);
  settings.addParm("SimpleTimeShowerCustomized:recoilWeight", 0.0, true, false, 0.0, 1.0);
  settings.addParm("SimpleTimeShowerCustomized:angleBias", 0.0, true, false, 0.0, 1.0);
  settings.addParm("SimpleTimeShowerCustomized:alphaModifier", 0.0, true, false, 0.0, 1.0);
}

void SimpleTimeShowerCustomized::init( BeamParticle* beamAPtrIn,
  ...
  // If SimpleSpaceShower does dipole recoil then SimpleTimeShowerCustomized must adjust.
...
bool SimpleTimeShowerCustomized::limitPTmax( Event& event, double Q2Fac, double Q2Ren) {
...
int SimpleTimeShowerCustomized::shower( int iBeg, int iEnd, Event& event, double pTmax,
  int nBranchMax) {
  ...
  do {
    ...
    double pTtimes = pTnext_custom( event, pTmax, 0.);
...
int SimpleTimeShowerCustomized::showerQED( int i1, int i2, Event& event, double pTmax) {
...
void SimpleTimeShowerCustomized::prepareProcess( Event& process, Event&, vector<int>&) {
...
void SimpleTimeShowerCustomized::prepareGlobal( Event& event) {
...
void SimpleTimeShowerCustomized::prepare( int iSys, Event& event, bool limitPTmaxIn) {
...
void SimpleTimeShowerCustomized::rescatterUpdate( int iSys, Event& event) {
...
// Update dipole list after each ISR emission (so not used for resonances).
  ...
  void SimpleTimeShowerCustomized::update( int iSys, Event& event, bool hasWeakRad) {
...
void SimpleTimeShowerCustomized::setupQCDdip( int iSys, int i, int colTag, int colSign,
...
void SimpleTimeShowerCustomized::regenerateOniumDipoles(Event &event) {
...
void SimpleTimeShowerCustomized::setupQEDdip( int iSys, int i, int chgType, int gamType,
...
void SimpleTimeShowerCustomized::setupWeakdip( int iSys, int i, int weakType,
  Event& event, bool limitPTmaxIn) {
...
void SimpleTimeShowerCustomized::setupWeakdipExternal(Event& event, bool limitPTmaxIn) {
...
void SimpleTimeShowerCustomized::setupHVdip( int iSys, int i, int colvSign,
  Event& event, bool limitPTmaxIn) {
...
double SimpleTimeShowerCustomized::pTnext( Event& event, double pTbegAll,
  double pTendAll, bool isFirstTrial, bool doTrialIn) {

  // std::cout << "- this is SimpleTimeShowerCustomized::pTnext" << std::endl;
  // Begin loop over all possible radiating dipole ends.
  dipSel  = 0;
  iDipSel = -1;
...
double SimpleTimeShowerCustomized::pTnextResDec() {
...
double SimpleTimeShowerCustomized::noEmissionProbability( double pTbegAll,
  ...
  for (int i=0; i < nTrialsMax; ++i) {
    ...
    while ( true ) {
      ...
      // Get trial shower pT.
      double pTtrial = pTnext( dipEnd, state, startingScale, minScale, m2dip,
        idA, type, s, x);
...
double SimpleTimeShowerCustomized::pTnext( vector<TimeDipoleEnd> dipEnd, Event event,
  ...
  for (int iDipEnd = 0; iDipEnd < int(dipEnd.size()); ++iDipEnd) {
    ...
    TimeDipoleEnd* dipEndNow      = &dipEnd[iDipEnd];
...
void SimpleTimeShowerCustomized::pT2nextQCD(double pT2begDip, double pT2sel,
  TimeDipoleEnd& dip, Event& event) {

  // std::cout << "- this is SimpleTimeShowerCustomized::pT2nextQCD" << std::endl;
...
void SimpleTimeShowerCustomized::pT2nextQED(double pT2begDip, double pT2sel,
  TimeDipoleEnd& dip, Event& event) {
...
void SimpleTimeShowerCustomized::pT2nextWeak(double pT2begDip, double pT2sel,
  TimeDipoleEnd& dip, Event& event) {
...
void SimpleTimeShowerCustomized::pT2nextHV(double pT2begDip, double pT2sel,
...
void SimpleTimeShowerCustomized::pT2nextOnium(double pT2begDip, double pT2sel,
  TimeDipoleEnd& dip, Event& event) {
...
bool SimpleTimeShowerCustomized::branch( Event& event, bool isInterleaved) {
...
bool SimpleTimeShowerCustomized::resonanceShower(Event& process, Event& event,
  ...
  do {
    ...
    double pTtimes  = (doFSRinResonances) ? pTnext_custom( event, pTmax, pTmerge)
      : -1.;
...
bool SimpleTimeShowerCustomized::initEnhancements() {
  enhanceFactors.clear();
  if( enhanceFSR.empty() ) {
...
bool SimpleTimeShowerCustomized::initUncertainties() {

  // Populate lists of uncertainty variations for SimpleTimeShowerCustomized, by keyword.
...
void SimpleTimeShowerCustomized::calcUncertainties(bool accept, double pAccept,
...
bool SimpleTimeShowerCustomized::rescatterPropagateRecoil( Event& event, Vec4& pNew) {
...
void SimpleTimeShowerCustomized::findMEtype( Event& event, TimeDipoleEnd& dip) {
...
int SimpleTimeShowerCustomized::findMEparticle( int id, bool isHiddenColour) {
...
double SimpleTimeShowerCustomized::gammaZmix( Event& event, int iRes, int iDau1,
  int iDau2) {
...
double SimpleTimeShowerCustomized::findMEcorr(TimeDipoleEnd* dip, Particle& rad,
...
double SimpleTimeShowerCustomized::calcMEcorr( int kind, int combiIn, double mixIn,
...
double SimpleTimeShowerCustomized::findMEcorrWeak(TimeDipoleEnd* dip,Vec4 rad,
...
void SimpleTimeShowerCustomized::findAsymPol( Event& event, TimeDipoleEnd* dip) {
...
double SimpleTimeShowerCustomized::calcPTresDec(Particle& res) {
...
void SimpleTimeShowerCustomized::list() const {

  // Header.
  cout << "\n --------  PYTHIA SimpleTimeShowerCustomized Dipole Listing  -----------"
       << "------------------------------------------------------- \n \n  "
  ...
  // Done.
  cout << "\n --------  End PYTHIA SimpleTimeShowerCustomized Dipole Listing  -------"
       << "-------------------------------------------------------" << endl;
...
void SimpleTimeShowerCustomized::loadCustomSettings() {
  zBias = parm("SimpleTimeShowerCustomized:zBias");
  coherenceFactor = parm("SimpleTimeShowerCustomized:coherenceFactor");
  azimuthalBias = parm("SimpleTimeShowerCustomized:azimuthalBias");
  recoilWeight = parm("SimpleTimeShowerCustomized:recoilWeight");
  angleBias = parm("SimpleTimeShowerCustomized:angleBias");
  alphaModifier = parm("SimpleTimeShowerCustomized:alphaModifier");
}

//==========================================================================
// Get Splitting z (Direct Access to getSplitSel() or dipEnd)
//==========================================================================

// double SimpleTimeShowerCustomized::getSplittingZ() {
//     if (getSplitSel() != nullptr) {
//         return getSplitSel()->z();
//     }
//     if (!dipEnd.empty()) {
//         return dipEnd.back().z;
//     }
//     return 0.0;
// }

//==========================================================================
// Compute Sudakov Suppression (Custom Coherence Term)
//==========================================================================

double SimpleTimeShowerCustomized::computeSudakov(double theta, double z) {
    double coherenceSuppression = exp(-coherenceFactor * theta);
    if (z < 0.1) {
        coherenceSuppression *= exp(-coherenceFactor * theta);
    }
    return exp(-coherenceSuppression);
}

//==========================================================================
// Modified pTnext() - Include Coherence Effects and Biasing
//==========================================================================

double SimpleTimeShowerCustomized::pTnext_custom(Event& event, double pTbegAll,
                                          double pTendAll, bool isFirstTrial,
                                          bool doTrialIn) {

  cout << "- this is SimpleTimeShowerCustomized::pTnext_custom" << endl;
  double pT = pTnext(event, pTbegAll, pTendAll, isFirstTrial, doTrialIn);
  if (pT <= 0.0)
    return 0.0;
  ...
  double theta = sqrt(1.0 - z) * sin(pT / event[0].e());
  double Delta = computeSudakov(theta, z);

  if (rndmPtr->flat() > Delta)
    return 0.0;
...
// //==========================================================================
// // Modified Kinematics - Include Angular and Azimuthal Biasing
// //==========================================================================
// 
// bool SimpleTimeShowerCustomized::kinematics(int iSys, int radPos,
//                                             int emtPos, int recPos,
//                                             Vec4& pRadBef, Vec4& pEmt,
//                                             Vec4& pRec) {
//     bool status = TimeShower::kinematics(iSys, radPos, emtPos, recPos,
//                                          pRadBef, pEmt, pRec);
//     if (!status) return false;
// 
//     double z = dipSel->z;
// 
//     double theta = sqrt(1.0 - z) * sin(pRadBef.pT() / pRadBef.e());
//     theta = pow(theta, angleBias);
// 
//     double phi = 2.0 * M_PI * rndmPtr->flat();
//     phi += azimuthalBias * sin(2.0 * phi);
// 
//     Vec4 q = pRadBef - pEmt;
//     pRec = recoilWeight * q + (1.0 - recoilWeight) * pRadBef;
// 
//     return true;
// }
