...
class PythiaOTFENC(object):
    ...
    def simulate(self, pythia):
        ...
        while iev < self.nev:
            ...
            # )
            parts = vector[fj.PseudoJet](
                [part_w_charge(p) for p in pythia.event if p.isFinal()]
            )
            self.evw = pythia.info.weight()
            self.analyze_event(parts, pythia.info.pTHat())
    ...
    def analyze_jet(self, jet):
        ...
        constituents_ch = [c for c in constituents if c.user_index() != 0]

        for p1, p2 in permutations(constituents_ch, 2):
            q1 = p1.user_index()
            q2 = p2.user_index()
