...
def part_w_charge(pyp):
    ...
    return pj
def part_w_status(pyp):
    pj = fj.PseudoJet(pyp.px(), pyp.py(), pyp.pz(), pyp.e())
    pj.set_user_index(int(pyp.status()))
    return pj
...
class PythiaOTFENC(object):
    ...
    def prepare(self):
        ...
        self.hists['status'] = ROOT.TH1F("status", "status", 200, 0, 200)
    ...
    def simulate(self, pythia):
        ...
        while iev < self.nev:
            ...
            self.analyze_event(parts_ch, parts_sc, pythia.info.pTHat())
    ...
    def analyze_event(self, parts, parts_sc, pthat):
