...
class JetAnalysisRoot(BaseAnalysis):
    ...
    def build_psjv_run3(self, event_struct):
    ...
    def accept_event_run2(self, event_struct):
        if event_struct is None:
            return False
        if not isinstance(event_struct, GenericObject):
            return False
    ...
    def process_event(self, event_struct):
        if not self._accept_event(event_struct):
            return
        self._build_psjv(event_struct)
        self.analysis(event_struct)
